...
##########
# Exercise 
# 1. Open `wrangling_student.R` in `R`. You may use `wrangling_complete.R` for troubleshooting if needed. 
# 2. Session -> Set Working Directory -> To Source File Location. 
##########

########## Clear the Workspace

# Whenever you begin working in R, it is usually (note that I said usually and not always) a good idea to clear your workspace. 
rm(list = ls())

########## Load Packages
# After clearing the workspace, the general convention is to first load in any packages that you intend to use in your script.
# 
...
# One reason that data exploration and manipulation can be time consuming is that simple calculations can often reveal annoying data errors: 
...
# We will use the 'gsub' function to replace all occurrences of a regex pattern with 
# a new pattern.
# 
# The blackslash \ helps us escape special behavior.
# 
# Now we can define a function to remove all dollar signs in the price and cast this column to a double:
...
listings %>% 
  summarise(avg_price = mean(nprice))
...
# The `n()` function gives a count of how many rows we have in each group, and `median()` does what you would expect.
 
########## 
# Exercise: Add two columns, num_listings and med_price that calculate the number of listings and median price by neighborhood.
########## 
...
# and even though the syntax will be foreign, let's take a sneak peak at what we can do with tidy data.
...
# The takeaway is that by putting our data in tidy format, creating an interesting, interpretable
# plot with very readable code is simple and fast.
...
# First, let's use our tidyverse tools to make the dataset we're interested in working with.
...
# We are gluing each new element together with `+` signs.   Clean, intuitive, and on the way to being pretty. 
...
# > That `factor` essentially tells ggplot to treat `bedrooms` as categorical rather than numeric.
...
# Another way to save a plot is to use `ggsave()`, which saves the last plot by default, for example: 
...
# Only new tools here are the `reorder` function we used in the `x` aesthetic, 
# which simply reorders the first argument in order by the last argument, and the `coord_flip` function, 
# which is neat theme function for flipping the x and y axes for readability. I prefer to use this function
# at the end of a plotting command to avoid confusion with other functions that take `x` and `y` arguments.
# 
...
# (Is this a central-Boston vs. peripheral-Boston effect? What would be some good ways to visualize that?)
# - Does this ordering vary over time, or is the South Boston Waterfront always the most high end?
...
# Need a couple extra dimensions in your plot? Try **faceting**, i.e. breaking out your plot 
...
# nothing (`.`) on the y-axis, against `room_type` on the x-axis.  
# Sometimes we have too many facets to fit on one line, and we want to let ggplot do the work of wrapping them in a nice way.  
# For this we can use `facet_wrap()`.

######### Connecting tidy data to `ggplot`

# `ggplot` and the grammar of graphics are very nice on their own, but the integration with the tidyverse is
# where the magic happens. By magic, I mean a framework for organizing exploratory data analysis that
# reduces the time between acquiring data and generating both interesting insights and presentable plots.

# `ggplot` and the tidyverse allow us to cleanly map our data to a visualization.  
# Let's say we want the distributions of daily, weekly, and monthly price, 
# with the color of the line showing which type of price it is.  Before we were able to do 
...
# But with the long format data, we can simply specify the color of our line with the `price_type` 
...
#' We are going to continue with that AirBnB data set
...
#' Let's first take a look at a for-loop (no exercise)
...
########## 
# Exercise: #
# Part a: Working with your partner, inspect the data in the `data/listings` directory. Write a similar command to read in this data. 
# Part b: Generalize the last two code chunks by writing a function that reads in all files in a given directory and returns a single data frame. 
...
# Time to take a look to see what we have. The first version of our analysis question is: 
#
...
#' 	- `price_per` should be on the y-axis
#' 	- `geom_line()` is probably the way to go. 
#' 	- distinguish the series by using `aes(color = listing_id)`
prices %>%
  head(2000) %>% 
  ggplot() + 
  aes(x = date, 
...
# against the actual data for the first 3000 rows. Use geom_line() for both. 
# You'll need to use geom_line() twice, with a different y aesthetic in each,
# and you should consider using facet_grid to show each listing and its model
# in a separate plot.
########## 
